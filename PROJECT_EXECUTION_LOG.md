...
Locked Phase 18 Analytics/Experiments as a deterministic, privacy-aware analytics and self-experiment foundation. Added expanded feature scope covering metric registry, data completeness, analytics snapshots, comparisons, self-experiments, experiment templates, lifecycle stages, baselines, confounders, invalid states, lessons learned, insight quality, provenance, causation guardrails, repository boundaries, trend/correlation/comparison engines, minimum data rules, chart boundaries, analytics dashboard, domain cards, insight inbox, action-from-insight preview, experiment lab UI, Carnos analytics explanation boundary, analytics-to-memory boundary, privacy/sensitivity labels, anti-demo-data audit, manual correction readiness, time range consistency, analytics preferences readiness, export-ready report boundary, and final fixtures/audit/report. Boundary preserved: no fake analytics, no silent AI writes, no hidden memory use, no Phase 19/20/21 leakage.

## Phase 18A-B — Companion Persistence Proof Audit + Level 4 Offline AI Readiness Lock

Locked Carnos persistence and offline AI readiness before Phase 18B. Added the rule that Carnos must not be one-chat-only and must support close/reopen and login-to-login continuity through Supabase/Postgres as online source of truth, IndexedDB or equivalent encrypted local cache as offline continuity layer, an offline sync queue, conflict detection, duplicate prevention, cached approved memories, cached analytics snapshots, offline experiment/log capture, online/offline mode routing, and Carnos cached-context honesty. Locked Level 4 Full Offline AI Companion as an advanced readiness target using Option C local AI server/runtime on MacBook Pro M3 Pro, with Ollama-compatible localhost boundary first, MLX/llama.cpp-compatible future runtime paths, local embeddings/retrieval readiness, future Tauri packaging, and browser-only local AI as fallback only. Boundary preserved: no false claim that full local runtime is implemented yet.
...
Added experiment repository boundary contracts. This includes template lookup boundaries, experiment read boundaries, draft capture preview boundaries, measurement read boundaries, confounder read boundaries, lesson summary read boundaries, memory candidate lesson read boundaries, evaluation preview boundaries, offline capture, sync queue readiness, source routing, cached/unsynced/deterministic labels, Phase 18B schema source-map requirement before runtime implementation, Carnos experiment repository explanation limits, and no-fake-experiment guardrails. Boundary preserved: no schema writes, no runtime SQL reads, no runtime data writes, no Supabase client calls, no fake experiment data, no sync queue implementation, no memory writes, no action execution, and no local runtime requirement during checks.
...
Added optional local Carnos runtime boundary and offline AI adapter contract. This includes runtime modes, adapter kinds, readiness states, offline disclosures, deterministic fallback, Option C local runtime boundary, future localhost-compatible runtime path, future MLX-compatible path, future llama.cpp-compatible path, future Tauri sidecar path, browser fallback limitation, cached/unsynced/deterministic/current-info disclosures, and review-before-memory-write preservation. Boundary preserved: no schema writes, no runtime SQL reads, no Supabase client calls, no network calls, no model calls, no streaming calls, no embedding calls, no vector search, no runtime install requirement, no runtime health call, no fake analytics data, no fake experiment conclusions, no memory writes, no action execution, and no local runtime requirement during checks.

## Anti-Demo-Data + Privacy/Sensitivity Audit

Added Phase 18 anti-demo-data and privacy/sensitivity audit. This covers anti-fake analytics, anti-fake experiment conclusions, analytics privacy/sensitivity, self-experiment privacy/sensitivity, Carnos explanation privacy/sensitivity, local runtime privacy/sensitivity, source-state disclosures, cached/stale/partial/missing/unsynced/deterministic preview disclosures, no-causality and no-proof claims, review-before-memory-write, no action execution, no model calls, no network calls, no Supabase runtime calls, no schema writes, no embeddings/vector search, and local-runtime-not-required checks.
...
Defined retention policy taxonomy, retention subjects, retention state model, future timestamp fields, retention versus privacy rules, retention action rules, warning codes, blocked reasons, audit events, badge requirements, memory candidate retention, saved memory retention, export retention, private session retention, privacy review request retention, audit preview retention, connector action retention, Spotify action retention, source/evidence retention, and custom tracker privacy proposal retention. No schema was needed because this chunk does not add persistence, migrations, RLS, repositories, retention jobs, cron workers, archive mutations, expiry mutations, or runtime enforcement.
