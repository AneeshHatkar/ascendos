...
export function OperatingDashboardCard({ card, children, footer }: OperatingDashboardCardProps) {
  const shouldShowEmptyState = card.status === 'empty' && !children;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${statusClassNameByStatus[card.status]}`}>
      ...
      <div className='mt-5'>
        {shouldShowEmptyState ? (
          <EmptyState
            ...
            description={card.emptyStateDescription ?? 'This card will populate when matching SQL-backed records exist.'}
          />
        ) : (
          children
        )}
